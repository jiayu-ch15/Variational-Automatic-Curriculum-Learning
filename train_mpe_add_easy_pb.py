...
def produce_uniform_case(num_case,boundary,num_agents,num_boxes):
    one_starts_landmark = []
    one_starts_agent = []
    one_starts_box = []
    archive = [] 
    for j in range(num_case):
        ...
        for i in range(num_boxes):
            box_location = np.random.uniform(-boundary, +boundary, 2) 
...
def main():
    ...
    for episode in range(episodes):
        ...
        starts_easy = produce_uniform_case(num_easy,boundary_easy,num_agents,num_boxes)
        starts_uniform = produce_uniform_case(num_uniform,boundary,num_agents,num_boxes)
        starts = starts_easy + starts_uniform
        # reset env 
        obs = envs.new_starts_obs_pb(starts, num_agents, num_boxes, args.n_rollout_threads)
