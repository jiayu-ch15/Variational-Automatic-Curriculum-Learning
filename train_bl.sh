...
CUDA_VISIBLE_DEVICES=0 python train_bl.py --env_name ${env} --algorithm_name ${algo} --scenario_name ${scenario_name} --num_agents ${num_agents} --num_boxes ${num_boxes} --task_type ${task_type} --seed ${seed} --floor_size ${floor_size} --grid_size ${grid_size}  --n_rollout_threads 300 --episode_length 60 --env_horizon 60 --num_mini_batch 1  --num_env_steps 200000000 --ppo_epoch 15 --attn --save_interval 50 --eval --data_chunk_length 40 \
