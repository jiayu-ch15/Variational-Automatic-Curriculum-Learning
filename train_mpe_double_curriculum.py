...
class node_buffer():
    ...
    def move_nodes(self, one_length, Rmax, Rmin, use_child_novelty, use_parent_novelty, child_novelty_threshold, del_switch, writer, timestep): 
        ...
        if use_child_novelty and len(child2archive)!=0:
            ...
            # self.archive_novelty = novelty(self.archive,self.archive)
            # archive,archive_novelty = novelty_sort(archive,archive_novelty)
            # mean_novelty = np.mean(self.archive_novelty)
...
def main():
    ...
    curriculum_episode_last = 0
    curriculum_episode_now = 0
    one_length_last = 0
    ...
    for episode in range(episodes):
        ...
        if use_novelty_sample:
            if last_node.agent_num!=0:
                last_node.childlist += last_node.SampleNearby_novelty(last_node.parent,logger, curriculum_episode_last * args.episode_length * one_length_last + curriculum_episode_now * args.episode_length * one_length_now)
            now_node.childlist += now_node.SampleNearby_novelty(now_node.parent,logger, curriculum_episode_last * args.episode_length * one_length_last + curriculum_episode_now * args.episode_length * one_length_now)
        else:
            if last_node.agent_num!=0:
        ...
        one_length_last = 0
        if last_node.agent_num!=0:
            starts_last, one_length_last = last_node.sample_starts(N_child,N_archive)
        ...
        for times in range(eval_frequency):
            ...
            if last_node.agent_num!=0:
                ...
                obs = envs.new_starts_obs(starts_last, last_node.agent_num, one_length_last)
                #replay buffer
                ...
                # replay buffer init
                if args.share_policy: 
                    share_obs = obs.reshape(one_length_last, -1)        
                ...
                for step in range(args.episode_length):
                    ...
                    # rearrange action
                    actions_env = []
                    for i in range(one_length_last):
                        one_hot_action_env = []
                        for agent_id in range(last_node.agent_num):
                    ...
                    obs, rewards, dones, infos, _ = envs.step(actions_env, one_length_last, last_node.agent_num)
                ...
                logger.add_scalars('agent/traing_cover_rate',
                                    {'training_cover_rate': np.mean(infos)}, 
                                    (curriculum_episode_last * args.episode_length * one_length_last + curriculum_episode_now * args.episode_length * one_length_now))
                curriculum_episode_last += 1
                ...
                if args.share_policy: 
                    ...
                    for agent_id in range(now_node.agent_num):
                        rollouts_now[agent_id].insert(share_obs, 
                                ...
                                np.array(masks)[:,agent_id])
            # import pdb;pdb.set_trace()
            logger.add_scalars('agent/traing_cover_rate',{'training_cover_rate': np.mean(infos)},  curriculum_episode_last * args.episode_length * one_length_last + curriculum_episode_now * args.episode_length * one_length_now)
            curriculum_episode_now += 1
            ...
            if args.share_policy:
                ...
                #     curriculum_episode * args.episode_length * one_length_last)
        ...
        if last_node.agent_num!=0:
            ...
            last_node.move_nodes(one_length_last, Rmax, Rmin, use_child_novelty, use_parent_novelty, child_novelty_threshold, del_switch, logger, curriculum_episode_last * args.episode_length * one_length_last + curriculum_episode_now * args.episode_length * one_length_now)
        now_node.eval_score = now_node.eval_score / eval_frequency
        now_node.move_nodes(one_length_now, Rmax, Rmin, use_child_novelty, use_parent_novelty, child_novelty_threshold, del_switch, logger, curriculum_episode_last * args.episode_length * one_length_last + curriculum_episode_now * args.episode_length * one_length_now)
        ...
        if (episode+1) % save_node_frequency ==0 and save_node_flag:
            last_node.save_node(save_node_dir, curriculum_episode_last + curriculum_episode_now)
            now_node.save_node(save_node_dir, curriculum_episode_last + curriculum_episode_now)
                ...
                if args.share_policy: 
                    ...
                    for agent_id in range(now_node.agent_num):
                        rollouts[agent_id].insert(share_obs, 
                                ...
                                np.array(masks)[:,agent_id])
            # import pdb;pdb.set_trace()
            logger.add_scalars('%iagent/cover_rate' %now_node.agent_num,{'cover_rate': np.mean(infos)}, curriculum_episode_last * args.episode_length * one_length_last + curriculum_episode_now * args.episode_length * one_length_now)
        ...
        total_num_steps = curriculum_episode_last * args.episode_length * one_length_last + curriculum_episode_now * args.episode_length * one_length_now
