...
def main():
    args = get_config()
    run = wandb.init(project='amigo',name=str(args.algorithm_name) + "_seed" + str(args.seed))
    ...
    node = node_buffer(args.num_agents,2000,
                           ...
                           epsilon=0,
                           delta=0)
    easy_cases = node.produce_good_case(num_case=args.n_rollout_threads, start_boundary=start_boundary, now_agent_num=args.num_agents)
    ...
    for episode in range(episodes):
        ...
        for times in range(eval_frequency):
            # add easy cases
            if episode == 0:
                obs = envs.new_starts_obs(easy_cases, args.num_agents, args.n_rollout_threads)
            else:
            ...
            for step in range(args.episode_length):
                ...
                current_reached_goal = current_reached_goal[:,0:num_agents*2] # agent state
        ...
        if episode % check_frequency==0:
            ...
            test_initial_state = test_initial_state[:,num_agents*2:] # landmark states
