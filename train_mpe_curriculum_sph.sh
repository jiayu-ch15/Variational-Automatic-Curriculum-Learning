...
num_agents=4
# algo="check"
algo='diversified_novelty_mid_sp3'
seed_max=1
...
# done
seed=1
CUDA_VISIBLE_DEVICES=3 python train_mpe_curriculum_sph.py --env_name ${env} --algorithm_name ${algo} --scenario_name ${scenario} --num_agents ${num_agents} --num_landmarks ${num_landmarks} --seed ${seed} --n_rollout_threads 500 --num_mini_batch 2 --episode_length 70 --num_env_steps 80000000 --ppo_epoch 15 --recurrent_policy --use_popart --use-max-grad-norm
