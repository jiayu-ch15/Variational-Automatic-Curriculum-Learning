    ...
    def produce_good_case(self, num_case, start_boundary_quadrant, now_seeker_num, now_ramp_num, fixed_ramp=False):
        ...
        for j in range(num_case):
            ...
            if fixed_ramp:
                ...
                # start_boundary_quadrant = [{'x':[18,26],'y':[16,16]},{'x':[12,12],'y':[1,11]}] 
                for i in range(now_ramp_num):
                    poses = []
                    for region_id in range(len(start_boundary_quadrant)):
                        if start_boundary_quadrant[region_id]['x'][0] != start_boundary_quadrant[region_id]['x'][1]:
                            x_pos = np.random.randint(start_boundary_quadrant[region_id]['x'][0], start_boundary_quadrant[region_id]['x'][1])
                        else:
                            x_pos = start_boundary_quadrant[region_id]['x'][0]
                        if start_boundary_quadrant[region_id]['y'][0] != start_boundary_quadrant[region_id]['y'][1]:
                            y_pos = np.random.randint(start_boundary_quadrant[region_id]['y'][0], start_boundary_quadrant[region_id]['y'][1])
                        else:
                            y_pos = start_boundary_quadrant[region_id]['y'][0]
                        poses.append(np.array([x_pos, y_pos]))
                    ramp_location = poses[np.random.randint(0, len(start_boundary_quadrant))] 
            ...
            while add_num < self.reproduction_num:
                for k in range(len_start):
                    ...
                    for i in range(s_len):
                        ...
                        # boundary_seeker = [{'x':[1,13],'y':[1,13]},{'x':[1,13],'y':[15,28]},{'x':[15,28],'y':[15,28]}]
                        # boundary_ramp = [{'x':[1,11],'y':[1,11]},{'x':[1,11],'y':[15,26]},{'x':[15,26],'y':[15,26]}]
                        if i < self.agent_num:
                            boundary = self.boundary_seeker
                        else:
                            boundary = self.boundary_ramp
                        # clip to boundary
                        st[i][0] = min(boundary[2]['x'][1],max(boundary[0]['x'][0],st[i][0]))
                        st[i][1] = min(boundary[2]['y'][1],max(boundary[0]['y'][0],st[i][1]))
                        if st[i][0] <= boundary[0]['x'][1]: # 在左半侧:
                            if st[i][1] > boundary[0]['y'][1] and st[i][1] < boundary[1]['y'][0]: # 在夹缝中
                                st[i][1] = random.sample([boundary[0]['y'][1],boundary[1]['y'][0]],1)[0]
                        elif st[i][0] >= boundary[2]['x'][0]: # 在右半侧
                            if st[i][1] < boundary[2]['y'][0]:
                                st[i][1] = np.random.randint(boundary[2]['y'][0],boundary[2]['y'][1])
                        else:
                            st[i][0] = boundary[0]['x'][1]

                    if len(self.parent_all) > self.topk + 1 and rejection_num <= 2 * self.reproduction_num:
...
def main():
    ...
    for episode in range(episodes):
        ...
        # region env reset
        args.num_ramps = now_ramp_num
        # 重新生成环境
        envs.close()
        envs = make_parallel_env(args)
        num_ramps = args.num_ramps
        actor_critic.base.num_agents = num_seekers     
        ...
        for times in range(eval_frequency):
            ...
            for step in range(args.episode_length):
                ...
                rollouts.insert(share_obs, 
                                ...
                                rewards, 
                                masks)
            wandb.log({'training_success_rate': np.mean(step_cover_rate[:, -historical_length:])}, current_timestep)
        ...
        if episode % args.eval_interval == 0 and args.eval:
            ...
            for key in order_obs:
                if key in eval_env.observation_space.spaces.keys():
                    space = list(eval_env.observation_space[key].shape)
                    if len(space)<2:  
                        space.insert(0,1)        
                    now_obs_space.append(space)
                    obs_dim += reduce(lambda x,y:x*y,space)
            ...
            print('1ramp_cover_rate: ', sum_cover_rate)
            wandb.log({str(1) + 'ramp_test_success_rate': sum_cover_rate}, current_timestep)

            test_reward = np.mean(rollouts.rewards)
            wandb.log({'test_reward': test_reward}, current_timestep)

            # region PC
            if next_phase_signal >= upper_bound:
                if now_ramp_num > ramp_nums_target:
                    torch.save({
                            'model': actor_critic
                            }, 
                            str(save_dir) + "/pc_model_%i_%i"%(num_seekers,num_boxes) + ".pt")
                    args.num_ramps = int(now_ramp_num/2)
                    now_ramp_num = args.num_ramps
                    # 重新生成环境
                    envs.close()
                    eval_env.close()
                    envs = make_parallel_env(args)
                    ...
                    actor_critic.base.encoder_critic.embedding.split_shape = now_obs_space[1:]
                    # update node buffer
                    last_node = node_buffer(num_seekers,num_boxes, now_ramp_num, buffer_length,
                                        archive_initial_length=args.n_rollout_threads,
                                        reproduction_num=M,
                                        max_step=max_step,
                                        start_boundary_quadrant=start_boundary_quadrant,
                                        boundary_seeker=boundary_seeker,
                                        boundary_ramp=boundary_seeker)
                    print('[run agent PC]: now ramp num: ', args.num_ramps)
            # endregion
