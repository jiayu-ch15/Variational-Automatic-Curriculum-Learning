...
class node_buffer():
    def __init__(self,agent_num,buffer_length,archive_initial_length,reproduction_num,max_step,start_boundary,boundary):
        ...
        self.archive = self.produce_good_case_grid_near(archive_initial_length, start_boundary, self.agent_num)
        # self.archive = self.produce_uniform_case(archive_initial_length, start_boundary, self.agent_num)
    ...
    def produce_good_case_grid_near(self, num_case, start_boundary, now_agent_num):
        # agent_size=0.1
        cell_size = 0.2
        grid_num = int(start_boundary * 2 / cell_size)
        grid = np.zeros(shape=(grid_num,grid_num))
    ...
    def sample_starts(self, N_child, N_archive):
        ...
        print('starts_archive: ', len(self.choose_archive_index))
        print('starts_child: ', len(self.choose_child_index))
            ...
            for times in range(eval_frequency):
                ...
                if use_uniform:
                    ...
                    if not deterministic_flag:
                        ...
                        logger.add_scalars('agent/deterministic_cover_rate',{'deterministic_cover_rate': np.mean(np.mean(step_cover_rate[:,-historical_length:],axis=1))}, current_timestep)
                        print('eval_cover_rate: ', np.mean(np.mean(step_cover_rate[:,-historical_length:],axis=1)))
