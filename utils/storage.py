...
class RolloutStorage_critic_k(object):
    def __init__(self, num_agents, episode_length, n_rollout_threads, critic_k, obs_space, action_space,
        ...
        self.recurrent_hidden_states = np.zeros((
            episode_length + 1, n_rollout_threads, num_agents, critic_k, recurrent_hidden_state_size)).astype(np.float32)
        self.recurrent_hidden_states_critic = np.zeros((
            episode_length + 1, n_rollout_threads, num_agents, critic_k, recurrent_hidden_state_size)).astype(np.float32)
                       
        self.rewards = np.zeros((episode_length, n_rollout_threads, num_agents, 1)).astype(np.float32)
        # critic_k
        self.value_preds = np.zeros((episode_length + 1, n_rollout_threads, num_agents, critic_k, 1)).astype(np.float32)
        self.returns = np.zeros((episode_length + 1, n_rollout_threads, num_agents, critic_k, 1)).astype(np.float32)
        self.action_log_probs = np.zeros((episode_length, n_rollout_threads, num_agents, 1)).astype(np.float32)
                ...
                if use_gae:
                    ...
                    for step in reversed(range(self.rewards.shape[0])):
                        for critic_id in range(self.critic_k):
                            if use_popart:
                                delta = self.rewards[step,:,agent_id] + gamma * value_normalizer.denormalize(torch.tensor(self.value_preds[
                                step + 1,:,agent_id,critic_id])).cpu().numpy() * self.masks[step + 1,:,agent_id] - value_normalizer.denormalize(torch.tensor(self.value_preds[step,:,agent_id,critic_id])).cpu().numpy()
                                gae = delta + gamma * gae_lambda * self.masks[step + 1,:,agent_id] * gae
                                gae = gae * self.bad_masks[step + 1,:,agent_id]
                                self.returns[step,:,agent_id,critic_id] = gae + value_normalizer.denormalize(torch.tensor(self.value_preds[step,:,agent_id,critic_id])).cpu().numpy()
                            else:
                                delta = self.rewards[step,:,agent_id] + gamma * self.value_preds[
                                    step + 1,:,agent_id,critic_id] * self.masks[step + 1,:,agent_id] - self.value_preds[step,:,agent_id,critic_id]
                                gae = delta + gamma * gae_lambda * self.masks[step + 1,:,agent_id] * gae
                                gae = gae * self.bad_masks[step + 1,:,agent_id]
                                self.returns[step,:,agent_id,critic_id] = gae + self.value_preds[step,:,agent_id,critic_id]
                else:
                    self.returns[-1,:,agent_id] = next_value
                    for step in reversed(range(self.rewards.shape[0])):
                        for critic_id in range(self.critic_k):
                            if use_popart:
                                self.returns[step,:,agent_id,critic_id] = (self.returns[step + 1,:,agent_id,critic_id] * \
                                gamma * self.masks[step + 1,:,agent_id] + self.rewards[step,:,agent_id]) * self.bad_masks[step + 1,:,agent_id] \
                                + (1 - self.bad_masks[step + 1,:,agent_id]) * value_normalizer.denormalize(torch.tensor(self.value_preds[step,:,agent_id,critic_id])).cpu().numpy()
                            else:
                                self.returns[step,:,agent_id,critic_id] = (self.returns[step + 1,:,agent_id,critic_id] * \
                                    gamma * self.masks[step + 1,:,agent_id] + self.rewards[step,:,agent_id]) * self.bad_masks[step + 1,:,agent_id] \
                                    + (1 - self.bad_masks[step + 1,:,agent_id]) * self.value_preds[step,:,agent_id,critic_id]
            else:
                if use_gae:
                    self.value_preds[-1,:,agent_id] = next_value
                    gae = 0
                    for step in reversed(range(self.rewards.shape[0])):
                        for critic_id in range(self.critic_k):
                            if use_popart:
                                ...
                                delta = self.rewards[step,:,agent_id] + gamma * self.value_preds[step + 1,:,agent_id,critic_id] * self.masks[step + 1,:,agent_id] - self.value_preds[step,:,agent_id,critic_id]
                                gae = delta + gamma * gae_lambda * self.masks[step + 1,:,agent_id] * gae
                                self.returns[step,:,agent_id,critic_id] = gae + self.value_preds[step,:,agent_id,critic_id]
                else:
                    self.returns[-1,:,agent_id] = next_value
    ...
    def feed_forward_generator_share(self, advantages, num_mini_batch=None, mini_batch_size=None, critic_k=1, critic_id=1):
        episode_length, n_rollout_threads, num_agents = self.rewards.shape[0:3]
        batch_size = n_rollout_threads * episode_length * num_agents
        ...
        if critic_k==1:
            ...
            share_obs = self.share_obs[:-1].reshape(-1, *self.share_obs.shape[3:])
            obs = self.obs[:-1].reshape(-1, *self.obs.shape[3:])
            recurrent_hidden_states = self.recurrent_hidden_states[:-1,:,:,critic_id].reshape(-1, self.recurrent_hidden_states.shape[-1])
            recurrent_hidden_states_critic = self.recurrent_hidden_states_critic[:-1,:,:,critic_id].reshape(-1, self.recurrent_hidden_states_critic.shape[-1])
            actions = self.actions.reshape(-1, self.actions.shape[-1])
            value_preds = self.value_preds[:-1,:,:,critic_id].reshape(-1, 1)
            returns = self.returns[:-1,:,:,critic_id].reshape(-1, 1)
            masks = self.masks[:-1].reshape(-1, 1)
            high_masks = self.high_masks[:-1].reshape(-1, 1)
            action_log_probs = self.action_log_probs.reshape(-1, 1)
...
class RolloutStorage_share(object):  # 减小share_obs
    def __init__(self, num_agents, episode_length, n_rollout_threads, obs_space, action_space,
        ...
        obs_shape = obs_space.shape
        self.share_obs = np.zeros((episode_length + 1, n_rollout_threads, obs_shape[0] * num_agents)).astype(np.float32)
        self.obs = np.zeros((episode_length + 1, n_rollout_threads, num_agents, obs_shape[0])).astype(np.float32)
    ...
    def shared_compute_returns(self,
                        ...
                        use_proper_time_limits=True,
                        use_popart=True,
                        value_normalizer=None): # 把所有的agent的return同时计算完
        if use_proper_time_limits:
            if use_gae:
...
class RolloutStorage_share_time(object):  # 减小share_obs
    def __init__(self, num_agents, episode_length, n_rollout_threads, obs_space, action_space,
                 recurrent_hidden_state_size, use_same_dim=False):
        
        obs_shape = obs_space.shape[0] + 1
        self.share_obs = np.zeros((episode_length + 1, n_rollout_threads, obs_shape * num_agents)).astype(np.float32)
        self.obs = np.zeros((episode_length + 1, n_rollout_threads, num_agents, obs_shape)).astype(np.float32)
