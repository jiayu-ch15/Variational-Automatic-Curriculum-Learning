...
class node_buffer():
    ...
    def SampleNearby_novelty(self, parents, writer, timestep): # produce high novelty children and return 
        if len(self.parent_all) > self.topk:
            ...
            novelty_threshold = np.max(self.parent_all_novelty)
        else:
            novelty_threshold = 0
            ...
            while add_num < self.reproduction_num:
                for i in range(len_start):
                    ...
                    if self.get_novelty([st],self.parent_all) > novelty_threshold:
                        ...
                        add_num += 1
            child_new = random.sample(child_new, self.reproduction_num)
            return child_new
...
def main():
    ...
    for episode in range(episodes):
        ...
        if use_novelty_sample:
            last_node.childlist += last_node.SampleNearby_novelty(last_node.parent,logger,curriculum_episode * args.episode_length * one_length)
        else:
            last_node.childlist += last_node.SampleNearby(last_node.parent)
        
        # reset env 
        # one length = now_process_num
        starts, one_length = last_node.sample_starts(N_child,N_archive)
        last_node.eval_score = np.zeros(shape=one_length)

        for times in range(eval_frequency):
            obs = envs.new_starts_obs(starts, num_agents, one_length)
            #replay buffer
            rollouts = RolloutStorage(num_agents,
                        args.episode_length, 
                        one_length,
            ...
            # replay buffer init
            if args.share_policy: 
                share_obs = obs.reshape(one_length, -1)        
            ...
            for step in range(args.episode_length):
                ...
                obs, rewards, dones, infos, _ = envs.step(actions_env, one_length, num_agents)
            ...
            logger.add_scalars('agent/traing_cover_rate',{'training_cover_rate': np.mean(infos)}, curriculum_episode * args.episode_length * one_length)
            ...
            if args.share_policy:
                ...
                logger.add_scalars('average_episode_reward',
                    ...
                    curriculum_episode * args.episode_length * one_length)
        ...
        last_node.move_nodes(one_length, Rmax, Rmin, use_child_novelty, use_parent_novelty, child_novelty_threshold, del_switch, logger, (episode+1) * args.episode_length * one_length)
                ...
                if args.share_policy: 
                    ...
                    for agent_id in range(num_agents):
                        rollouts[agent_id].insert(share_obs, 
                                ...
                                np.array(masks)[:,agent_id])
            # import pdb;pdb.set_trace()
            logger.add_scalars('agent/cover_rate',{'cover_rate': np.mean(infos)}, curriculum_episode * args.episode_length * one_length)

        total_num_steps = curriculum_episode * args.episode_length * one_length
