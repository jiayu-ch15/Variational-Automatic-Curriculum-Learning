...
class node_buffer():
    def __init__(self,agent_num, box_num, buffer_length,archive_initial_length,reproduction_num,max_step,start_boundary,boundary,boundary_quadrant):
        ...
        self.boundary_quadrant = boundary_quadrant
    ...
    def SampleNearby_novelty(self, parents, child_novelty_threshold, writer, timestep): # produce high novelty children and return 
        #print(">> start SampleNearby_novelty")
        #wandb.log({str(self.agent_num)+'parents all length': len(self.parent_all)},timestep)
            ...
            while add_num < self.reproduction_num:
                ...
                for i in range(len_start):
                    ...
                    for i in range(s_len):
                        ...
                        epsilon_x = random.randint(1,self.max_step) * delta_x_direction
                        epsilon_y = random.randint(1,self.max_step) * delta_y_direction
                        if i < self.agent_num:
                            st[i][0] = min(max(st[i][0] + epsilon_x,self.boundary_quadrant[0]),self.boundary_quadrant[1])
                            st[i][1] = min(max(st[i][1] + epsilon_y,self.boundary_quadrant[2]),self.boundary_quadrant[3])
                        else:
    ...
    def sample_starts(self, N_child, N_archive, N_parent=0):
        ...
        print("[sample_archive]: %d [sample_childlist]: %d [sample_parent]: %d"%(len(self.choose_archive_index), len(self.choose_child_index), len(self.choose_parent_index)))
    ...
    def sample_starts_wo_evaluation(self, N_archive, N_parent):
        ...
        for i in range(len(self.choose_parent_index)):
            starts.append(self.parent_all[self.choose_parent_index[i]])
        print("[sample_archive]: %d [sample_parent]: %d"%(len(self.choose_archive_index), len(self.choose_parent_index)))
        return starts, one_length, starts_length
...
def main():
    args = get_config()
    run = wandb.init(project='box_locking',name=str(args.algorithm_name) + "_seed" + str(args.seed))
    ...
    for episode in range(episodes):
        ...
        if use_samplenearby:
            if use_novelty_sample:
                last_node.archive += last_node.SampleNearby_novelty(last_node.parent, child_novelty_threshold, logger, curriculum_timestep)
            else:
        ...
        for times in range(eval_frequency):
            ...
            step_lock_rate = np.zeros(shape=(one_length, args.episode_length))
            step_return_rate = np.zeros(shape=(one_length, args.episode_length))
            step_success_rate = np.zeros(shape=(one_length, args.episode_length))
            ...
            for step in range(args.episode_length):
                ...
                rollouts.insert(share_obs, 
                                ...
                                rewards, 
                                masks)
            wandb.log({'training_lock_rate': np.mean(step_lock_rate[:, -historical_length:])}, curriculum_timestep)
            wandb.log({'training_return_rate': np.mean(step_return_rate[:, -historical_length:])}, curriculum_timestep)
            wandb.log({'training_success_rate': np.mean(step_success_rate[:, -historical_length:])}, curriculum_timestep)
            curriculum_timestep += args.episode_length * starts_length
            curriculum_episode += 1
            #last_node.eval_score += np.mean(step_return_rate[:,-historical_length:],axis=1)
            last_node.eval_score += np.mean(step_success_rate[:,-historical_length:], axis=1)
            with torch.no_grad(): 
                for agent_id in range(num_agents):         
                    if args.share_policy: 
                        actor_critic.eval()                
            ...
            if args.share_policy:
                ...
                for agent_id in range(num_agents):
                    ...
                    wandb.log({'reward': np.mean(rollouts.rewards[:,:,agent_id])},
                        (episode + 1) * args.episode_length * args.n_rollout_threads)                                                 
            # clean the buffer and reset
            rollouts.after_update()
        end1 = time.time()
        print('[step_update_time]: %.2f'%(end1- start1))
        # move nodes
        last_node.eval_score = last_node.eval_score / eval_frequency
        if use_samplenearby:
            last_node.move_nodes_wo_evaluation(one_length, Rmax, Rmin, del_switch, curriculum_timestep)
        ...
        print("[last_node_parent]: %d [childlist]: %d [archive]: %d"%(len(last_node.parent), len(last_node.childlist),  len(last_node.archive)))
        ...
        if episode % args.eval_interval == 0 and args.eval:
            ...
            test_success_rate_mean = np.mean(test_success_rate[:, -historical_length:])
            wandb.log({'test_lock_rate': test_lock_rate_mean}, curriculum_timestep)
            wandb.log({'test_return_rate': test_return_rate_mean}, curriculum_timestep)
            wandb.log({'test_success_rate': test_success_rate_mean}, curriculum_timestep)
            print("[test_lock_rate]: %.2f [test_return_rate]: %.2f [test_success_rate]: %.2f"%(test_lock_rate_mean, test_return_rate_mean, test_success_rate_mean))
