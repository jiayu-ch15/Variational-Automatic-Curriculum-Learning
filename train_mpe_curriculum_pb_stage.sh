...
num_agents=2
num_boxes=2
algo='phase_pb_4agent0.98_step0.2'
# algo='check'
seed_max=1
...
# done
seed=3
CUDA_VISIBLE_DEVICES=0 python train_mpe_curriculum_pb_stage.py --env_name ${env} --algorithm_name ${algo} --scenario_name ${scenario} --num_agents ${num_agents} --num_landmarks ${num_landmarks} --num_boxes ${num_boxes}  --seed ${seed} --n_rollout_threads 500 --num_mini_batch 8 --episode_length 120 --num_env_steps 600000000 --ppo_epoch 15 --recurrent_policy --use_popart
