...
def generate_initial_goals(num_case, start_boundary, num_agents, num_landmarks):
    pos_dim = 2  #坐标维度
    one_starts_agent = np.zeros((num_agents, pos_dim), dtype = float)
    one_starts_landmark = np.zeros((num_landmarks, pos_dim), dtype = float)
    goals = np.zeros((num_case, (num_agents + num_landmarks)*pos_dim), dtype = float)
    for j in range(num_case):
        ...
        indices = random.sample(range(num_landmarks), num_agents)
        for i, k in enumerate(indices):
            epsilon = -2 * 0.01 * random.random() + 0.01
            one_starts_agent[i] = one_starts_landmark[k]+epsilon
...
def main():
    ...
    test_num_agents = args.num_agents
    mean_cover_rate = 0
    ...
    gan_configs['goal_center'] = np.zeros((num_agents + num_landmarks)* 2, dtype=float)
    gan_configs['goal_size'] = (num_agents + num_landmarks)*2
    gan = StateGAN(gan_configs = gan_configs, state_range=gan_configs['goal_range'])
    feasible_goals = generate_initial_goals(num_case = 10000, start_boundary = start_boundary, num_agents = num_agents, num_landmarks = num_landmarks)                            
    ...
    # add easy cases
    # easy_goals = generate_initial_goals(num_case = args.n_rollout_threads, start_boundary = start_boundary, agent_num = args.num_agents)
    ...
    for episode in range(episodes):
        ...
        # generate the starts
        starts = numpy_to_list(goals, list_length=num_envs, shape=(num_agents + num_landmarks,2))
