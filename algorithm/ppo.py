...
class PPO(): # actor_critic合并
    def __init__(self,                 
        ...
        self.num_stop_step = self.num_mini_batch
        ...
        # self.optimizer_dist = optim.Adam(actor_critic.dist.parameters(), lr=lr, eps=eps, weight_decay=weight_decay)
    ...
    def update_double_share(self, last_agents, current_agents, rollouts_last, rollouts_current, turn_on=True):
        advantages_last = []
        advantages_current = []
                ...
                if self.use_clipped_value_loss:
                    if self.use_huber_loss:
                        if self.use_popart:
                            ...
                            value_losses = huber_loss(error,self.huber_delta)
                            value_loss = weight_last * torch.max(value_losses, value_losses_clipped).mean()
                        else:
                            value_pred_clipped = value_preds_batch_last + (values - value_preds_batch_last).clamp(-self.clip_param, self.clip_param)
                        ...
                        if self.use_popart:
                            ...
                            value_loss = weight_last * 0.5 * torch.max(value_losses, value_losses_clipped).mean()
                        else:
                            value_pred_clipped = value_preds_batch_last + (values - value_preds_batch_last).clamp(-self.clip_param, self.clip_param)
                    ...
                    if self.use_huber_loss:
                        if self.use_popart:
                            ...
                            error = return_batch_last - values
                        value_loss = weight_last * huber_loss(error,self.huber_delta).mean()
                    else:
                        if self.use_popart:
                            value_loss = weight_last * 0.5 * (self.value_normalizer(return_batch_last) - values).pow(2).mean()
                        else:
                            value_loss = weight_last * 0.5 * (return_batch_last - values).pow(2).mean()               
                ...
                if self.use_clipped_value_loss:
                    if self.use_huber_loss:
                        if self.use_popart:
                            ...
                            value_losses = huber_loss(error,self.huber_delta)
                            value_loss = weight_current * torch.max(value_losses, value_losses_clipped).mean()
                        else:
                            value_pred_clipped = value_preds_batch_current + (values - value_preds_batch_current).clamp(-self.clip_param, self.clip_param)
                        ...
                        if self.use_popart:
                            ...
                            value_loss = weight_current * 0.5 * torch.max(value_losses, value_losses_clipped).mean()
                        else:
                            value_pred_clipped = value_preds_batch_current + (values - value_preds_batch_current).clamp(-self.clip_param, self.clip_param)
                    ...
                    if self.use_huber_loss:
                        if self.use_popart:
                            ...
                            error = return_batch_current - values
                        value_loss = weight_current * huber_loss(error,self.huber_delta).mean()
                    else:
                        if self.use_popart:
                            value_loss = weight_current * 0.5 * (self.value_normalizer(return_batch_current) - values).pow(2).mean()
                        else:
                            value_loss = weight_current * 0.5 * (return_batch_current - values).pow(2).mean()               
...
class PPO_MLPbase():# actor_critic分开
    def __init__(self,                 
                 actor_critic,
                 clip_param,
                 ppo_epoch,
        ...
        self.actor_critic = actor_critic
        self.optimizer_actor = optim.Adam(actor_critic.base.actor.parameters(), lr=lr, eps=eps, weight_decay=weight_decay)
        self.optimizer_critic = optim.Adam(actor_critic.base.critic.parameters(), lr=lr, eps=eps, weight_decay=weight_decay)
    ...
    def update_double_share(self, last_agents, current_agents, rollouts_last, rollouts_current, timesteps):
        advantages_last = []
        advantages_current = []
        for agent_id in range(last_agents):
            if self.use_popart:
    ...
    def update_single(self, agent_id, rollouts, timestep, warm_up=False):
        advantages = []
        if self.use_popart:
            advantages = rollouts.returns[:-1,:] - self.value_normalizer.denormalize(torch.tensor(rollouts.value_preds[:-1,:])).cpu().numpy()
        else:
        ...
        for e in range(self.ppo_epoch):
            ...
            for sample in data_generator: 
                ...
                if self.use_accumulate_grad: # 几个batch都不optimize，直到batch更新完再step
                    ...
                    if self.use_max_grad_norm:
                        nn.utils.clip_grad_norm_(self.actor_critic.base.actor.parameters(), self.max_grad_norm)
...
class PPO4():# actor_critic分开, k个critic网络
    def __init__(self,                 
        ...
        self.eps = eps
        self.weight_decay = weight_decay
        self.critic_k = len(self.actor_critic.critic_base)

        # self.optimizer = optim.Adam(actor_critic.parameters(), lr=lr, eps=eps, weight_decay=weight_decay)
        self.optimizer_actor = optim.Adam(actor_critic.actor_base.parameters(), lr=lr, eps=eps, weight_decay=weight_decay)
        self.optimizer_critic = []
        for i in range(self.critic_k):
            self.optimizer_critic.append(optim.Adam(actor_critic.critic_base[i].parameters(), lr=lr, eps=eps, weight_decay=weight_decay))
        self.use_popart = use_popart
        if self.use_popart:
            self.value_normalizer = PopArt(1, device=self.device)
        else:
                ...
                for sample in data_generator: 
                    ...
                    if self.use_accumulate_grad: # 几个batch都不optimize，直到batch更新完再step
                        if count_stop_step >= self.num_mini_batch or count_stop_step==0:
                            if actor_update:
                                self.optimizer_actor.zero_grad()
                            self.optimizer_critic[critic_id].zero_grad()
                            count_stop_step = 0
                        ...
                        if count_stop_step == self.num_mini_batch-1: 
                            self.optimizer_critic[critic_id].step()
                            if actor_update:
                                self.optimizer_actor.step()
                        count_stop_step += 1
                    else:
