...
class PPO(): # actor_critic合并
    ...
    def update_double_share(self, last_agents, current_agents, rollouts_last, rollouts_current, turn_on=True):
        ...
        for e in range(self.ppo_epoch):
            ...
            for sample_last, sample_current in zip(data_generator_last, data_generator_current): 
                ...
                # last agent update
                self.actor_critic.agents_num = last_agents
                ...
                self.actor_critic.agents_num = current_agents
    ...
    def update_double_share_pb(self, last_agents, current_agents, rollouts_last, rollouts_current, turn_on=True):
        advantages_last = []
        advantages_current = []
        for agent_id in range(last_agents):
            if self.use_popart:
        ...
        for e in range(self.ppo_epoch):
            ...
            for sample_last, sample_current in zip(data_generator_last, data_generator_current): 
                ...
                # last agent update
                self.actor_critic.agents_num = last_agents
                self.actor_critic.boxes_num = last_agents
                values, action_log_probs, dist_entropy, _, _ = self.actor_critic.evaluate_actions(agent_id, share_obs_batch_last, 
                obs_batch_last, recurrent_hidden_states_batch_last, recurrent_hidden_states_critic_batch_last, masks_batch_last, high_masks_batch_last, actions_batch_last)
                ...
                # current agent backward
                self.actor_critic.agents_num = current_agents
                self.actor_critic.boxes_num = current_agents
                values, action_log_probs, dist_entropy, _, _ = self.actor_critic.evaluate_actions(agent_id, share_obs_batch_current, 
                obs_batch_current, recurrent_hidden_states_batch_current, recurrent_hidden_states_critic_batch_current, masks_batch_current, high_masks_batch_current, actions_batch_current)
...
class PPO3():# actor_critic分开
    ...
    def update_share_asynchronous(self, num_agents, rollouts, warm_up=False, initial_optimizer=False):
        advantages = []
        for agent_id in range(num_agents):
            if self.use_popart:
                advantage = rollouts.returns[:-1,:,agent_id] - self.value_normalizer.denormalize(torch.tensor(rollouts.value_preds[:-1,:,agent_id])).cpu().numpy()
    ...
    def update_double_share_pb(self, last_agents, current_agents, rollouts_last, rollouts_current):
        advantages_last = []
        advantages_current = []
        for agent_id in range(last_agents):
            if self.use_popart:
...
class PPO4():# actor_critic分开, k个critic网络
    ...
    def update_share_asynchronous(self, num_agents, rollouts):
        ...
        for e in range(self.ppo_epoch):
            ...
            actor_update_id = np.random.randint(self.critic_k)
            actor_update = False
            for critic_id in range(self.critic_k):
                actor_update = False
                if critic_id == actor_update_id:
                    actor_update = True
