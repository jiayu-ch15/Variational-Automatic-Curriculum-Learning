...
def main():
    ...
    obs, _ = envs.reset(now_agent_num)
    ...
    for episode in range(episodes):
        ...
        for step in range(args.episode_length):
            ...
            obs, rewards, dones, infos, _ = envs.step(actions_env, args.n_rollout_threads, now_agent_num)
        ...
        logger.add_scalars('agent%i/cover_rate_1step' % now_agent_num,{'cover_rate_1step': np.mean(step_cover_rate[:,-1])},(episode + 1) * args.episode_length * args.n_rollout_threads)
        logger.add_scalars('agent%i/cover_rate_5step' % now_agent_num,{'cover_rate_5step': np.mean(np.mean(step_cover_rate[:,-5:],axis=1))},(episode + 1) * args.episode_length * args.n_rollout_threads)
        mean_cover_rate = np.mean(np.mean(step_cover_rate[:,-5:],axis=1))
        print('mean_cover_rate: ', mean_cover_rate)
        ...
        if args.share_policy:
            actor_critic.train()
            value_loss, action_loss, dist_entropy = agents.update_share(now_agent_num, rollouts)
                           
            rew = []
            for i in range(rollouts.rewards.shape[1]):
                rew.append(np.sum(rollouts.rewards[:,i]))
            logger.add_scalars('%iagents_average_episode_reward' % now_agent_num,
                {'%iagents_average_episode_reward' % now_agent_num: np.mean(rew)},
        ...
        if mean_cover_rate > upper_bound:
            mean_cover_rate = 0
            last_agent_num = now_agent_num
            now_agent_num = min(target_num, last_agent_num*2)
            add_num = now_agent_num - last_agent_num
            if add_num!=0:
                actor_critic.agents_num = now_agent_num
                if now_agent_num==8:
                    agents.num_mini_batch = 16
                elif now_agent_num==16:
                    agents.num_mini_batch = 96
                elif now_agent_num==32:
                    agents.num_mini_batch = 256
                else:
                    agents.num_mini_batch = 640
                if args.share_policy:
                    torch.save({'model': actor_critic}, str(save_dir) + "/%iagent_model.pt" % now_agent_num)

        obs, _ = envs.reset(now_agent_num)
        rollouts = RolloutStorage_share(now_agent_num,
                    args.episode_length, 
                    args.n_rollout_threads,
                    envs.observation_space[0], 
                    envs.action_space[0],
