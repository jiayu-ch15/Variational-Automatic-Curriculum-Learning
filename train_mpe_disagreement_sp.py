...
from algorithm.ppo import PPO,PPO4
from algorithm.model import Policy,Policy4, ATTBase_add, ATTBase_actor_dist_add, ATTBase_critic_add

from config import get_config
from utils.env_wrappers import SubprocVecEnv, DummyVecEnv
from utils.util import update_linear_schedule
from utils.storage import RolloutStorage, RolloutStorage_critic_k
...
def main():
    args = get_config()
    run = wandb.init(project='disagreement',name=str(args.algorithm_name) + "_seed" + str(args.seed))
    ...
    if args.share_policy:
        actor_base = ATTBase_actor_dist_add(envs.observation_space[0].shape[0], envs.action_space[0], num_agents).to(device)
        critic_base = []
        for i in range(critic_k):
            critic_base.append(ATTBase_critic_add(envs.observation_space[0].shape[0], num_agents).to(device))
        actor_critic = Policy4(envs.observation_space[0], 
                    envs.action_space[0],
    ...
    for episode in range(episodes):
        ...
        starts_share_obs = np.expand_dims(starts_share_obs,1).repeat(num_agents,axis=1)
        starts_obs = obs.copy()
        starts_recurrent_hidden_states = np.zeros((buffer_capacity, num_agents, critic_k, args.hidden_size)).astype(np.float32)
        starts_recurrent_hidden_states_critic = np.zeros((buffer_capacity, num_agents, critic_k, args.hidden_size)).astype(np.float32)
        starts_masks = np.ones((buffer_capacity, num_agents, 1)).astype(np.float32)
        ...
        for step in range(args.episode_length):
            ...
            for i, done in enumerate(dones): 
                mask = []               
                for agent_id in range(num_agents):
                    for critic_id in range(critic_k): 
                        if done[agent_id]:    
                            recurrent_hidden_statess[agent_id][i][critic_id] = np.zeros(args.hidden_size).astype(np.float32)
                            recurrent_hidden_statess_critic[agent_id][i][critic_id] = np.zeros(args.hidden_size).astype(np.float32)    
                    if done[agent_id]:
                        mask.append([0.0])
                    else:
                        mask.append([1.0])
            ...
            if args.share_policy: 
                ...
                rollouts.insert(share_obs, 
                            obs, 
                            np.array(recurrent_hidden_statess).transpose(1,0,2,3), 
                            np.array(recurrent_hidden_statess_critic).transpose(1,0,2,3), 
                            np.array(actions).transpose(1,0,2),
                            np.array(action_log_probs).transpose(1,0,2), 
                            np.array(values).transpose(1,0,2,3),
                            rewards, 
                            masks)
            else:
                share_obs = []
        ...
        if args.share_policy:
            actor_critic.train()
            value_loss, action_loss, dist_entropy = agents.update_share_asynchronous(num_agents, rollouts) 
        ...
        if episode % check_frequency==0:
            ...
            #     save_90_flag = False
            print('test_agent_num: ', num_agents)
