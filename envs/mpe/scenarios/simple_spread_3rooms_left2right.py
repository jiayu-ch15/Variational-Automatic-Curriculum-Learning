...
class Scenario(BaseScenario):
    def make_world(self, args, now_agent_num=None):
        ...
        world.walls = [Wall(orient='V', axis_pos=-6.5, endpoints=(-6, 6),width=3.0, hard=True),
                ...
                Wall(orient='H', axis_pos=-2.5, endpoints=(-6, 6),width=3.0, hard=True),
                Wall(orient='H', axis_pos=2.5, endpoints=(-6, 6),width=3.0,hard=True),
                # up left wall
                Wall(orient='V', axis_pos=-2, endpoints=(0.8, 2), width=2, hard=True),
                # down left wall
                Wall(orient='V', axis_pos=-2, endpoints=(-2, 0.4), width=2, hard=True),
                # up right wall
                Wall(orient='V', axis_pos=2, endpoints=(-0.4, 2), width=2, hard=True),
                # down right wall
                Wall(orient='V', axis_pos=2, endpoints=(-2, -0.8), width=2, hard=True)
                ]
        # add agents
