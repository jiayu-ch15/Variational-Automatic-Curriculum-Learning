...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, make_callback=None, reward_callback=None,
        ...
        for agent in self.agents:
            ...
            if self.discrete_action_space:
                ...
                u_action_space = spaces.Box(low=-agent.u_range, high=+agent.u_range, shape=(self.world.dim_p,), dtype=np.float32)# [-1,1]
            if agent.movable:
            ...
            if self.discrete_action_space:
                ...
                c_action_space = spaces.Box(low=0.0, high=1.0, shape=(self.world.dim_c,), dtype=np.float32)# [0,1]
    ...
    def step(self, action_n):
        ...
        for i,agent in enumerate(self.agents):
            ...
            # print('share: ', shared_reward)
            # print('self_reward: ', self._get_reward(agent))
    ...
    def new_starts_obs_pb(self,starts_one, now_agent_num, now_box_num):
        ...
        self.reset_callback(self.world)
        self._reset_render()
        self.agents = self.world.policy_agents # people
        ...
        for i, agent in enumerate(self.agents_all): # agent+box
            ...
            agent.state.c = np.zeros(self.world.dim_c)
        for i, landmark in enumerate(self.world.landmarks):
            landmark.state.p_pos = starts_one[i+num_agents]
            landmark.state.p_vel = np.zeros(self.world.dim_p)
