...
import pdb
import torch
# from train_boxlocking_curriculum import node_buffer
from train_hidenseek_curriculum import node_buffer
import copy
...
class PolicyViewer_hs_single(MjViewer):
    ...
    def run(self):
        ...
        for i, key in enumerate(self.order_obs):
            if key in self.env.observation_space.spaces.keys():             
                if self.mask_order_obs[i] == None:          
                    temp_share_obs = self.dict_obs[key][1].reshape(self.num_agents,-1).copy()
                    temp_obs = temp_share_obs.copy()
                else:
                    temp_share_obs = self.dict_obs[key][1].reshape(self.num_agents,-1).copy()
                    temp_mask = self.dict_obs[self.mask_order_obs[i]][1].copy()
                    temp_obs = self.dict_obs[key][1].copy()
        ...
        self.share_obs.append(reshape_obs)   
        ...
        while self.duration is None or time.time() < self.end_time:
            ...
            #print(self.dict_obs['observation_self'])
            self.total_rew += rew
            self.obs = []
            self.share_obs = []   
            for i, key in enumerate(self.order_obs):
...
class PolicyViewer_hs_single_good_case(MjViewer):
    '''
    PolicyViewer runs a policy with an environment and optionally displays it.
        env - environment to run policy in
        policy - policy object to run
    ...
    def __init__(self, env, policies, display_window=True, seed=None, duration=None):
        ...
        last_node = node_buffer(last_seekers_num,last_box_num, last_ramp_num, buffer_length,
                            archive_initial_length=100,
                            reproduction_num=320,
                            max_step=max_step,
        ...
        self.starts = copy.deepcopy(last_node.archive)
