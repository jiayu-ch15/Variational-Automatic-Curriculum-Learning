...
class Wall:
    ...
    def split_for_doors(self, num_doors=1, door_size=1, all_connect=False,
                        random_state=np.random.RandomState(),fixed_door=False):
        '''
        ...
        for door in door_regions:
            ...
            # (inclusive boundary)
            if fixed_door:
                door_start = np.floor((diffs[door] - door_size + 1)/2).astype('int')
...
def split_walls(walls, door_size, random_state=np.random.RandomState(), fixed_door=False):
    '''
    ...
    doors = []
    for wall in walls:
        new_walls, new_doors = wall.split_for_doors(door_size=door_size, random_state=random_state,fixed_door=fixed_door)
...
class WallScenarios(EnvModule):
    ...
    def __init__(self, n_agents, grid_size, door_size, scenario, friction=None, p_door_dropout=0.0,
                 fixed_door=False, low_outside_walls=False):
        self.n_agents = n_agents
        self.fixed_door = fixed_door
    ...
    def build_world_step(self, env, floor, floor_size):
        ...
        new_walls, doors = split_walls(walls_to_split, self.door_size,
                                    random_state=env._random_state, fixed_door=self.fixed_door)
