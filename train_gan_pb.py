...
def generate_initial_goals(num_case, start_boundary, agent_num, box_num):
    pos_dim = 2  #坐标维度
    one_starts_agent = np.zeros((agent_num, pos_dim), dtype = float)
    one_starts_landmark = np.zeros((box_num, pos_dim), dtype = float)
    one_starts_boxes = np.zeros((box_num, pos_dim), dtype = float)
    goals = np.zeros((num_case, (agent_num + box_num*2)*pos_dim), dtype = float)
    for j in range(num_case):
        ...
        indices = random.sample(range(box_num), box_num)
        for j in indices:
            epsilon = -2 * 0.01 * random.random() + 0.01
            one_starts_boxes[j] = one_starts_landmark[j]+epsilon
        indices = random.sample(range(box_num), agent_num)
        for k in indices:
            epsilon = -2 * 0.01 * random.random() + 0.01
            one_starts_agent[k] = one_starts_boxes[k]+epsilon
        goals[j] = np.concatenate((one_starts_agent, one_starts_boxes, one_starts_landmark), axis=None)
    return goals
...
def main():
    args = get_config()
    run = wandb.init(project='goal_gan_pb',name=str(args.algorithm_name) + "_seed" + str(args.seed))
    ...
    gan_configs['goal_center'] = np.zeros((num_agents + num_boxes* 2)*2, dtype=float)
    gan_configs['goal_size'] = (num_agents + num_boxes*2)*2
    gan = StateGAN(gan_configs = gan_configs, state_range=gan_configs['goal_range'])
    feasible_goals = generate_initial_goals(num_case = 10000, start_boundary = start_boundary, agent_num = args.num_agents, box_num= num_boxes)                            
