...
from algorithm.ppo import PPO, PPO_2
...
class node_buffer():
    ...
    def produce_good_case(self, num_case, start_boundary_bottom,start_boundary_up, now_agent_num, now_box_num):
        ...
        for j in range(num_case):
            ...
            if now_box_num >= now_agent_num:
                indices = np.random.choice(range(now_box_num), now_agent_num, replace=False)
            else: 
                indices = np.random.choice(range(now_box_num), now_agent_num, replace=True)
            for k in indices:
    ...
    def produce_uniform_case(self, num_case, start_boundary_bottom,start_boundary_up, now_agent_num, now_box_num):
        one_starts_agent = []
        one_starts_box = []
        archive = [] 
        for j in range(num_case):
            ...
            for k in range(now_agent_num):
                agent_location = np.random.randint(start_boundary_bottom, start_boundary_up, 2)  
                one_starts_agent.append(copy.deepcopy(agent_location))
...
def main():
    ...
    now_obs_space = all_obs_space[0]  # now_obs_space作为数量变化后的参数
    
    # endregion

    # region init algorithm
    if args.share_policy:
        actor_critic = Policy(now_obs_space, 
                    all_action_space[0],
    ...
    for episode in range(episodes):
        ...
        for times in range(eval_frequency):
            ...
            if args.share_policy:
                actor_critic.train()
                if warm_up_flag:
                    if warm_up_count == warm_up_iters:
                        value_loss, action_loss, dist_entropy = agents.update_share_asynchronous(num_agents, rollouts, warm_up = False)
                    else:
                        value_loss, action_loss, dist_entropy = agents.update_share_asynchronous(num_agents, rollouts, warm_up = True)
                        warm_up_count += 1
                else:
        ...
        if episode % args.log_interval == 0:
            ...
            print("Scenario {} Algo {} updates {}/{} episodes, total num timesteps {}/{}, FPS {}.\n"
            ...
            print("now agent num: ", num_agents)
            print("now box num: ", num_boxes)
            if args.share_policy:
        ...
        if episode % args.eval_interval == 0 and args.eval:
            ...
            for i in range(len(infos)):
                test_success_rate += infos[i]['success_rate'] 
            test_success_rate = test_success_rate / len(infos)
            print('test_sucess_rate: ', test_success_rate)
            logger.add_scalars('agent/test_success_rate_%i_%i'%(num_agents,num_boxes),{'test_success_rate': test_success_rate}, curriculum_episode * args.episode_length * starts_length)
            logger.add_scalars('agent/test_success_rate', {'test_success_rate': test_success_rate}, curriculum_episode * args.episode_length * starts_length)            
            test_reward = np.mean(rollouts.rewards[:,:,agent_id])
            print('test_reward: ', test_reward)
            logger.add_scalars('agent/test_reward_%i_%i'%(num_agents,num_boxes),{'test_reward': test_reward}, curriculum_episode * args.episode_length * starts_length)
            logger.add_scalars('agent/test_reward', {'test_reward': test_reward}, curriculum_episode * args.episode_length * starts_length)

            # region PC
            upper_bound = 0.0
            if test_success_rate > upper_bound:
                if args.num_agents < agent_nums_max:
                    torch.save({
                            'model': actor_critic
                            }, 
                            str(save_dir) + "/pc_model_%i_%i"%(num_agents,num_boxes) + ".pt")
                    args.num_agents += 2
                    warm_up_count = 0
                    # 重新生成环境
                    envs.close()
                    ...
                    eval_env = make_eval_env(args,eval_num)
                    num_agents = args.num_agents
                    actor_critic.base.num_agents = num_agents
                    action_movement_dim = [3]*num_agents
                    # update policy base's obs_shape 
                    now_obs_space = []
                    obs_dim = 0
                    for key in order_obs:
                        if key in envs.observation_space.spaces.keys():
                            ...
                            if len(space)<2:  
                                space.insert(0,1)        
                            now_obs_space.append(space)
                            obs_dim += reduce(lambda x,y:x*y,space)
                    now_obs_space.insert(0, obs_dim)
                    actor_critic.base.obs_shape = now_obs_space
                    actor_critic.base.encoder_actor.embedding.split_shape = now_obs_space[1:]
                    actor_critic.base.encoder_critic.embedding.split_shape = now_obs_space[1:]
                    # update node buffer
                    last_node = node_buffer(num_agents, num_boxes, buffer_length,
                        archive_initial_length=args.n_rollout_threads,
                        reproduction_num=M,
                        max_step=max_step,
                        start_boundary_bottom=start_boundary_bottom,
                        start_boundary_up=start_boundary_up,
                        boundary=boundary)
                    print('[run agent PC]: now agent num: ', args.num_agents)

                elif args.num_boxes < box_num_max:
                    torch.save({
                            'model': actor_critic
                            }, 
                            str(save_dir) + "/pc_model_%i_%i"%(num_agents,num_boxes) + ".pt")
                    ...
                    last_node = node_buffer(num_agents, num_boxes, buffer_length,
                        ...
                        start_boundary_up=start_boundary_up,
                        boundary=boundary)
                    print('[run box PC]: now box num: ', args.num_boxes)
            # endregion
